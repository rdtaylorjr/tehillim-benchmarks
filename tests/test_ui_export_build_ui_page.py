...
def test_build_ui_page_html_fills_in_every_family_missing_from_the_input() -> None:
    ...
    assert set(data["families"]) == set(ALL_FAMILY_IDS)
    assert data["families"]["phonology"] == {
        "parallelism_overall": [],
        "parallelism_by_type": [],
        "genre_overall": [],
        "genre_by_genre": [],
...
def test_merge_family_json_files_keys_by_the_single_family_each_file_carries(tmp_path) -> None:
    ...
    merged = merge_family_json_files([semantic_path, lexical_path])
